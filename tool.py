...
# python tool.py evaluate --input input_tweets --output output_tweets --model models/Naive_Bayes.joblib --vectorizer models/vectorizer.joblib
parser_evaluate = subparsers.add_parser('evaluate')
parser_evaluate.add_argument('--model', required=True,
							  help='File path to Sklearn model in .joblib format.')
parser_evaluate.add_argument('--vectorizer', required=True,
							help='File path to Sklearn vectorizer in .joblib format.')
...
# python tool.py stream --topic="cats" --output output_tweets --model models/Naive_Bayes.joblib  --vectorizer models/vectorizer.joblib
...
parser_stream.add_argument('--model', required=True,
							help='File path to Sklearn model in .joblib format.')
parser_stream.add_argument('--vectorizer', required=True,
							help='File path to Sklearn vectorizer in .joblib format.')
...
parser_simulate.add_argument('--model', required=True,
							help='File path to Sklearn model in .joblib format.')
parser_simulate.add_argument('--vectorizer', required=True,
							help='File path to Sklearn vectorizer in .joblib format.')
...
if __name__ == '__main__':
	if args.subcommand == 'download':
		...
		evaluator = Evaluate(args.model, args.vectorizer, args.input, args.output)
		evaluator.execute()
	elif args.subcommand == 'stream':
		streamer = Stream(args.topic, args.model, args.vectorizer, args.output, datetime.timedelta(seconds=args.duration))
		streamer.execute()
	elif args.subcommand == 'simulate':
		simulator = StreamSimulator(args.model, args.vectorizer, args.input, args.output, datetime.timedelta(seconds=args.duration))
		simulator.execute()
	else:
